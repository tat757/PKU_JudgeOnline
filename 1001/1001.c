...
int main(){
	...
	int digit[NUM_INPUT];				//To save the digits of each numbers
	int i,j,k;							//To use for the loops

	/*To get the inputs*/
	for( i=0; i<6; i++){
		scanf("%s %d", baseStr[i], power[i]);
	}

	/*Transfer the base numbers from string to integer */
	for( i=0; i<6; i++){
		k=0;									//Use to store the integer
		for( j=0; baseStr[j]!='/0', j++){			//Loop untill hit the end of the string
